...
var matcher = function (targVal, srcVal, key) {
  ...
  if (_.isObject(targVal)) {
    ...
    if (/_\.[^\:]+/.test(targKeys[0])) {
      ...
      var keyLodashFn = _[keyLodashMatch[1]]
      if (! keyLodashFn) {
      ...
      try {
        var newSrcVal = keyLodashFn.apply(_, keyArgs);
      } catch (error) {
  ...
  var lodashMatchFn = lodashMatch[1] ? _[lodashMatch[1]] : null
  ...
  var currentIsMatch = lodashMatchFn ? lodashMatchFn.apply(_, matcherArgs) : targVal === srcVal
